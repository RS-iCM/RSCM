...
#include <math.h>
#include <time.h>
#include "powell_min.c"  // Containing functions for numerical optimization using Powell's method.

/*********************************************************************************************
  Notes:
    In C, memory for matrices is allocated such that rows occupy contiguous space.
    avec: the array storing the elements in the matrix.
...
/*********************************************************************************************
  ...
  Functions: (? in the name of the following functions can be 0/1.)
    err_fit_? :
...
void RSCM(double *pTbase, double *pk, double *pRUE, double *pSLA, double *pbeta1, double *peGDD,
             double *pd, double *pfmLAI, int *poOpt, int *pn_unknown, int *pn_VI, int *pn_wcol, int *pregressionOpt, int *pimpliedLAIOpt,
             int *pstart, int *pndoy, int *pnrecords, int *pnObs,
             double *para0vec, double *prior_inv_cov_vec, double *prior_mean_vec, double *coef_vec, double *reginvSigmavec,
             double *wx_data_vec, double *obs_d_vec,
             double *paraout, double *LAIobs){

  double Tbase,k,RUE,SLA,beta1,eGDD,d,fmLAI;
  int oOpt,n_unknown,n_VI,n_wcol,start,ndoy,nrecords,nObs,regressionOpt,impliedLAIOpt;
  double *para0, **prior_inv_cov, *prior_mean, **coef, **reg_inv_Sigma;
  ...
  void SS(int n_unknown, double *vpar, double *obj){
    ...
    double a,b,c,L0,rGDD,temp,temp2,TempVI;
    ...
    temp = 0;
    for(i=0;i<nrecords-2;i++){
      sDOY = floor(wxData[i+start][0]);
      if(sDOY==ODOY){
        if((impliedLAIOpt==0)&&(oOpt>1)){
          for(j=0;j<n_VI;j++){
            if(regressionOpt==0)
              TempRes[j] = err_fit_0(coef[j][0],coef[j][1],LAI[i+1],rs_data[iObs][j+1]);
            else if(regressionOpt==1)
              TempRes[j] = err_fit_1(coef[0][j],coef[1][j],LAI[i+1],rs_data[iObs][j+1]);
          }
          for(j=0;j<n_VI;j++){
            TempVI = 0;
            for(kk=0;kk<n_VI;kk++)
          ...
          temp += sqr(log(LAIobs[iObs])-log(LAI[i+1]));
        }
  ...
  wxData = ddmatrix(wx_data_vec,ndoy,n_wcol);
  ...
  */
  ...
  for(i=0;i<nObs;i++){
    ...
    if(bComplete){
      ...
      if(oOpt>1){
        ...
        if(regressionOpt==0)
          LAIobs[i] = reparam_0(paramLAI[1])<fmLAI ? reparam_0(paramLAI[1]) : fmLAI;
        else if(regressionOpt==1)
          LAIobs[i] = reparam_1(paramLAI[1])<fmLAI ? reparam_1(paramLAI[1]) : fmLAI;
      }else{
        LAIobs[i] = rs_data[i][1];
      }
    }
  }
  if(nNonEmptyObs != 0){
    GDDarr[0] = 0;
    for(i=0;i<=nrecords-2;i++){
      dGDD = (wxData[i+start][2]+wxData[i+start][3])/2-Tbase;
      GDDarr[i+1] = GDDarr[i] + (dGDD > 0 ? dGDD : 0);
      RADarr[i+start] = wxData[i+start][1];
    }
    ...
    if(!ifail){
      ...
      for(i=1;i<=n_unknown-1;i++){
        paraout[i-1] = 1/(1+exp(-param[i]));
      }
      paraout[n_unknown-1] = exp(param[n_unknown]);
    }
    count ++;
  }
  ...
  free(prior_inv_cov); free(coef); free(reg_inv_Sigma);
  free(wxData);
  free(rs_data);
}
